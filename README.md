...
서로 다른 멱등성 키로 동일한 imageUrl을 요청하면, 멱등성 키 기반 방어를 우회하여 중복 작업이 생성되는 문제가 있습니다.

**해결법:**
- imageUrl의 SHA-256 해시를 저장하고, 시간 윈도우(기본 3분) 내 동일 해시가 존재하면 409 Conflict를 반환합니다.
- 검증 순서는 imageUrl 해시 중복 검사 → 멱등성 키 검사 → 신규 생성의 2단계 검증으로 구성됩니다.

**DB 변경:**
- `image_url_hash CHAR(64)` 컬럼을 추가하여 SHA-256 해시를 저장합니다.
- `(image_url_hash, created_at)` 복합 인덱스로 시간 윈도우 내 중복 검색 성능을 확보합니다.

**설정:**
- `task.duplicate-image-url.window-minutes`로 중복 판정 시간 윈도우를 조정할 수 있습니다.
