...
PENDING ──→ PROCESSING ──→ COMPLETED
                 │
                 └──→ FAILED ──→ PENDING (재시도, retryCount < maxRetry)
                          └──→ [종료] (retryCount ≥ maxRetry)
```
...
- 완료된 작업의 멱등성 키는 배치 작업으로 30일 후 물리 삭제합니다.
...
**외부 서버 장애 해결법:**
Resilience4j의 4개 모듈을 조합합니다:

| 모듈 | 역할 | 핵심 설정 |
|---|---|---|
| **Circuit Breaker** | 연속 실패 시 호출 차단, 외부 서비스에 복구 시간 부여 | 실패율 50% 초과 시 OPEN, 30초 후 HALF_OPEN |
| **Retry** | 일시적 실패에 자동 재시도 | 최대 3회, Exponential Backoff (60s → 180s → 600s) + Jitter |
| **Rate Limiter** | 외부 서비스 호출 속도 사전 제한 | 초당 10건 |
| **Bulkhead** | 동시 호출 수 제한, 리소스 격리 | 동시 10건 |

적용 순서: `Retry → CircuitBreaker → RateLimiter → Bulkhead → 실제 호출`

**Exponential Backoff + Jitter:**
재시도 간격을 60s → 180s → 600초로 점차 늘리되, 랜덤 지연 시간인 지터를 추가하여 여러 클라이언트가 동시에 재시도하는 Thundering Herd 문제를 방지합니다.
재시도 시간은 요구사항의 기술 문서에서 이미지 서버의 예상 시간을 토대로 정의했습니다.
이 설정 또한 외부 값으로 분리하여 외부 서비스 상황에 따라 유연하게 변경할 수 있습니다.
...
코루틴 다수가 동시에 DB 커넥션을 요구하면 풀 고갈이 발생합니다. HikariCP 풀 사이즈를 적정 수준(20~50)으로 설정하고, 커넥션 획득 타임아웃을 짧게(5초) 설정하여 풀 고갈 시 빠르게 실패하도록 합니다.

**3순위: Polling 부하**
PROCESSING 상태 작업이 N개이면 Polling으로 초당 N/interval건의 GET 요청이 외부 서비스에 발생합니다. Polling 간격을 점진적으로 늘리고(2초 → 10초), 동시 Polling 수에 상한을 두어 제어합니다.
...
| `/mock/process` | POST | X-API-KEY | `{ imageUrl }` | `{ jobId, status }` | 400, 401, 422, 429, 500 |
| `/mock/process/{jobId}` | GET | X-API-KEY | - | `{ jobId, status, result }` | 401, 404, 500 |

**상태 모델 매핑:**
- Mock Worker: `PROCESSING`, `COMPLETED`, `FAILED` (3단계)
- 내부 시스템: `PENDING` 추가 → 4단계 (서버 재시작 정합성을 위해 필수)
...
- Thread-safe한 `AtomicReference<String>` 또는 `@Volatile var`로 저장하여 동시성 문제를 방지합니다.
...
여러 스레드가 동시에 401을 받는 경우, 한 스레드만 재발급하고 나머지는 대기하도록 합니다:
...
class ApiKeyManager(private val webClient: WebClient) {
    ...
    suspend fun getOrReissue(): String {
        // 401 발생 시 한 스레드만 재발급
        if (apiKey == null || needsReissue) {
            if (reissueLock.compareAndSet(false, true)) {
                try {
                    apiKey = issueNewKey()
                    logger.info("API Key reissued successfully")
                } finally {
        ...
        return apiKey ?: throw ApiKeyUnavailableException()
    }
}
```
...
| 장애 내성 | Resilience4j | 경량, 데코레이터 조합, Coroutine 지원 |
| DB 접근 | Spring Data JPA (Hibernate) | Optimistic Lock, Pageable 내장 |
| DB | MySQL 8.0 | UNIQUE 제약 + Gap Lock으로 원자적 중복 검사 |
| 테스트 | JUnit5, MockK, WireMock, Awaitility | Kotlin 친화, HTTP 시뮬레이션, 비동기 대기 |
